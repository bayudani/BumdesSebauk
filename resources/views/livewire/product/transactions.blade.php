            ...
            <div class="grid lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2">
                        ...
                        <div x-data="{
                                ...
                                <div class="flex lg:flex-col gap-3">
                                    <template x-for="media in mediaItems" :key="media.id">
                                        <div @click="mainDisplay = '{{ Storage::disk('public')->url('') }}/' + media.image; mainMediaType = media.media_type"
                                            ...
                                            :class="{ 'border-purple-600': mainDisplay.includes(media.image), 'border-transparent': !mainDisplay.includes(media.image) }">
                                            
                                            <template x-if="media.media_type === 'image'">
                                                <img :src="'{{ Storage::disk('public')->url('') }}/' + media.image" alt="{{ $product->name }}" class="w-full h-full object-cover rounded-md">
                                            </template>
                                            <template x-if="media.media_type === 'video'">
                                                <div class="relative w-full h-full bg-black rounded-md flex items-center justify-center">
                                                    <video :src="'{{ Storage::disk('public')->url('') }}/' + media.image" class="w-full h-full object-cover rounded-md"></video>
                                                    <div class="absolute text-white bg-black bg-opacity-50 rounded-full p-2 pointer-events-none">
                            ...
                            <div class="lg:col-span-4 w-full aspect-[4/3]">
                                <template x-if="mainMediaType === 'image'">
                                    <img :src="mainDisplay" alt="{{ $product->name }}" class="w-full h-full object-cover rounded-xl shadow-lg">
                                </template>
                                <template x-if="mainMediaType === 'video'">
                                    <video controls autoplay muted loop :src="mainDisplay" class="w-full h-full object-cover rounded-xl shadow-lg bg-black">
                    ...
                    <div class="space-y-4 border-t pt-6">
                        ...
                        <div>
                            <label for="customer_name" class="block text-sm font-medium text-slate-900 mb-1">Nama Lengkap</label>
                        ...
                        <div>
                            <label for="customer_phone" class="block text-sm font-medium text-slate-900 mb-1">No. Telepon / WA</label>
                        ...
                        <div>
                            <label for="customer_address" class="block text-sm font-medium text-slate-900 mb-1">Alamat Lengkap</label>
                ...
                <div class="lg:col-span-1 self-start lg:sticky top-8">
                    <div class="bg-gray-100 p-6 rounded-md space-y-8">
                         <div>
                            ...
                            @error('quantity') <div class="text-red-500 text-sm mt-1 text-right">{{ $message }}</div> @enderror
                            <ul class="text-slate-600 font-medium mt-6 space-y-3 border-t border-gray-300 pt-4">
                                ...
                                <li class="flex justify-between text-lg font-semibold text-slate-900 border-t border-gray-300 pt-3 mt-3">
                                    <span>Total</span>
                        ...
                        <div class="border-t pt-6">
                            ...
                            <div class="flex flex-col gap-3 mt-4">
                                <label class="flex items-center p-3 border bg-white rounded-lg cursor-pointer has-[:checked]:border-purple-500 has-[:checked]:ring-2 has-[:checked]:ring-purple-200">
                                    <input type="radio" wire:model="delivery_method" value="pickup" class="w-5 h-5">
                                    <div class="ml-3">
                                ...
                                <label class="flex items-center p-3 border bg-white rounded-lg cursor-pointer has-[:checked]:border-purple-500 has-[:checked]:ring-2 has-[:checked]:ring-purple-200">
                                    <input type="radio" wire:model="delivery_method" value="delivery" class="w-5 h-5">
                                    <div class="ml-3">
                            ...
                            @error('delivery_method') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                        </div>
                        <div class="border-t pt-6">
                            <h3 class="text-lg font-semibold text-slate-900">Pembayaran</h3>
                             <p class="text-slate-500 text-sm mt-2">Silakan transfer ke rekening tujuan, lalu upload bukti.</p>
                            <div class="flex flex-col gap-3 mt-4">
                                <label class="flex items-center p-3 border bg-white rounded-lg cursor-pointer has-[:checked]:border-purple-500 has-[:checked]:ring-2 has-[:checked]:ring-purple-200">
                            ...
                            <div class="mt-6">
                                ...
                                <input type="file" wire:model="proof_of_transaction" class="block w-full text-sm file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-purple-50 file:text-purple-700 hover:file:bg-purple-100">
                                @error('proof_of_transaction') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                                <div wire:loading wire:target="proof_of_transaction" class="text-sm text-gray-500 mt-1">Uploading...</div>
                            </div>
                        </div>
                        <button type="submit" class="w-full mt-6 py-3 text-lg font-medium bg-purple-600 text-white rounded-md hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 disabled:opacity-50" wire:loading.attr="disabled">
