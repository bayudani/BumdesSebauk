    ...
    <section id="for-you-entertainment" class="max-w-[1130px] mx-auto py-16 md:py-24 px-4">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center">
            ...
            <a href="{{ route('articles') }}"
                class="mt-4 md:mt-0 py-3 px-6 border border-gray-300 rounded-full font-semibold text-sm hover:bg-gray-100 transition-all">
                Lihat Semua Berita
            </a>
        </div>
            ...
            @if (isset($articles) && $articles->isNotEmpty())
                ...
                <div class="w-full lg:w-7/12">
                    <a href="{{ route('blog.show', $featured->slug) }}"
                        class="block relative overflow-hidden rounded-2xl group h-full min-h-[400px]">
                        <img src="{{ $featuredImage }}" alt="Gambar Artikel"
                            class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                        <div class="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent"></div>
                        <div class="absolute bottom-0 left-0 p-6 text-white">
                            <span class="text-sm bg-green-500 px-2 py-1 rounded">Featured</span>
                ...
                <div class="w-full lg:w-5/12 flex flex-col gap-5">
                    @foreach ($articles->skip(1)->take(3) as $article)
                        ...
                        <a href="{{ route('blog.show', $article->slug) }}"
                            class="flex items-center gap-4 p-4 border border-gray-200 rounded-2xl bg-white hover:shadow-lg hover:border-green-500 transition-all duration-300">
